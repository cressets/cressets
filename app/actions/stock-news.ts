...
async function scrapeStockNews(symbol: string) {
    try {
        ...
        if (symbol.endsWith('.KS') || symbol.endsWith('.KQ')) {
            ...
            const res = await fetch(naverNewsUrl);
            ...
            for (const item of items) {
                ...
                const source = item.officeNm;
                const time = item.dt; // 네이버는 이미 포맷된 문자열 ("2시간 전" 등)을 줄 때가 있음
                const url = `https://m.stock.naver.com/domestic/stock/${code}/news/view/${item.artId}`;
